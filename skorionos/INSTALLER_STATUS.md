...
> 最后更新: 2025-11-01 (第十二次更新)
> 
> 基于 GTK4 + Python 的现代化安装器，支持手柄操作、多设备适配、进度显示、本地安装、高级选项、统一日志系统、UI布局优化、智能回退机制、网络检测、离线模式、多系统安装、实时网速显示、自动时区检测、**完整取消机制** 🆕
...
│   ├── install_utils.py       # 安装后处理（Steam、post_install、时区复制 🆕）
│   ├── local_file_manager.py  # 本地文件扫描与挂载管理 🆕
│   ├── log_utils.py           # 日志清理和上传（fpaste）
│   └── timezone_utils.py      # 时区自动检测（IP地理定位 + 回退策略）🆕
│
├── network/                    # 网络管理模块
...
- ✅ **Confirm 页面**: 操作摘要显示
- ✅ **Bootstrap 页面**: frzr-bootstrap 执行、实时日志、错误处理、**Steam bootstrap 下载进度显示**、**本地文件扫描**、**完整取消机制** 🆕
- ✅ **Version 页面**: **安装方式选择（在线/本地）**、通道/桌面/NVIDIA 选择、**本地文件列表显示**、**高级选项开关**
- ✅ **Advanced 页面**: **固件覆盖、CDN 加速、备用源、Debug 模式**
- ✅ **Install 页面**: frzr-deploy 执行、**支持本地文件安装**、**应用高级选项配置**、进度显示、日志输出、**后安装配置**、**完整取消机制** 🆕
...
- ✅ **timezone_utils.py** 🆕:
  - `auto_detect_timezone()` - 自动检测时区（多层级回退策略）
  - `_detect_by_ip_geolocation()` - 通过 IP 地理定位检测时区（85-95% 准确度）
  - `_detect_by_systemd()` - 通过 systemd 推荐时区（回退方案）
  - `apply_timezone_to_live()` - 应用时区到 Live 环境
  - `get_current_timezone()` - 获取当前系统时区
  - 支持多个免费 API（ipapi.co、ip-api.com、worldtimeapi.org）
  - 自动验证时区有效性（检查 /usr/share/zoneinfo/）
...
- ✅ **install_utils.py**:
  ...
  - `copy_timezone_config()` - 复制时区配置到安装系统 🆕
    - 从环境变量或 timedatectl 读取时区
    - 创建 /etc/localtime 符号链接（指向 /usr/share/zoneinfo/）
    - 写入 /etc/timezone 文件
    - 验证时区文件有效性
  - `post_install()` - 后安装优化
    - 遍历所有 btrfs 部署子卷
...
- ✅ **三层时区获取机制**：
  0. **现有系统继承**（最高优先级）
     - Live ISO 启动时自动检测现有系统分区（frzr_root）
     - 复制 `/etc/timezone` 和 `/etc/localtime` 配置
     - 应用到 Live 环境并设置 `INSTALLER_TIMEZONE` 环境变量
     - 图形安装器检测到环境变量后跳过自动检测
     - **优势**：重装系统时保持时区一致，无需重新检测
  1. **中国国内优先检测**（第一优先级，避免代理误判）
     - 使用真正的中国国内 IP 服务：
       - 太平洋电脑网（whois.pconline.com.cn）
       - ip.useragentinfo.com
       - cip.cc
     - 检测用户是否在中国（包含"中国"或"China"关键字）
     - 如果在中国，直接返回 `Asia/Shanghai`（中国统一时区）
     - 超时时间：2 秒（快速失败）
     - 不受国际代理影响，更准确识别中国用户
  2. **国际 IP 地理定位**（第二优先级）
     - 使用多个免费 API（ipapi.co、ip-api.com、worldtimeapi.org）
     - 准确度：85-95%（单时区国家如日本、韩国等）
     - 自动回退：一个 API 失败自动尝试下一个
  3. **systemd 推荐**（第三优先级）
     - 从 `timedatectl` 读取系统推荐时区
  4. **默认 UTC**（最终回退）
     - 如果所有检测失败，使用 UTC

#### 应用流程
- ✅ **Live ISO 启动时（最优先）**：
  - `copy_system_configs` 函数检测现有系统分区
  - 如果找到 `frzr_root` 分区，复制时区配置
  - 应用到 Live 环境并设置环境变量
  - **优势**：重装系统时无缝继承时区，无需任何检测
  
- ✅ **网络连接后异步检测**（第二优先级）：
  - 检查 `INSTALLER_TIMEZONE` 环境变量
  - 如已设置（从现有系统复制）：跳过检测
  - 如未设置：在网络页面连接成功后，**后台异步启动**时区检测
  - 不阻塞安装流程，用户体验更好
  - 如果已经在线（如以太网），页面初始化时自动启动
  - 检测完成后自动刷新状态栏时间显示
  - 检测时间：< 3 秒（异步执行，不影响用户操作）
  
- ✅ **Bootstrap 阶段智能检查**（最后回退）：
  - 检查时区来源：现有系统 > 网络检测 > 同步检测
  - 如已获取：跳过，显示来源（如"✓ 时区已从现有系统获取: Asia/Shanghai"）
  - 如未获取：同步执行检测（网络未连接或检测失败的回退方案）
  
- ✅ **应用到 Live 环境**：
  - 使用 `timedatectl set-timezone` 设置系统时区
  - 刷新 Python 进程的时区缓存（`time.tzset()`）
  - 状态栏时间立即更新到正确时区
...
- ✅ **复制到目标系统**：
  ...
  - 创建 `/etc/localtime` 符号链接（指向 `/usr/share/zoneinfo/`）
  - 写入 `/etc/timezone` 文件
  - 与网络配置一起复制（Install 页面后安装阶段）

#### 准确度保障
- ✅ **时区验证**：检查 `/usr/share/zoneinfo/` 中是否存在对应文件
- ✅ **环境变量传递**：通过 `INSTALLER_TIMEZONE` 环境变量在安装流程中传递
- ✅ **日志记录**：完整记录检测过程和结果，方便排查问题
- ✅ **容错处理**：任何步骤失败都不会阻塞安装流程，只记录警告

#### 用户体验
- ✅ **完全自动化**：无需用户干预，开箱即用
- ✅ **状态栏实时更新**：时区变更后状态栏时间立即显示正确时区
- ✅ **透明日志**：日志中清晰显示检测到的时区（如 `✓ 检测到时区: Asia/Shanghai`）
- ✅ **不影响安装速度**：检测过程仅需 1-3 秒
...
**安全的取消机制**，允许用户在 Bootstrap 和 Install 阶段随时取消操作：

#### Install 阶段取消（frzr-deploy）

**执行的清理操作**：
1. ✅ **终止进程**：
   - 优雅终止（SIGTERM）→ 5秒超时 → 强制终止（SIGKILL）
   - 等待 3 秒让进程释放文件句柄
   
2. ✅ **卸载挂载点**（逆序卸载）：
   - `/tmp/frzr_root/boot/efi` → `/tmp/frzr_root/boot` → `/tmp/frzr_root`
   - Sync 确保数据写入
   - 多层回退：`umount` → `umount -f` → `umount -l`（lazy unmount）
...
**支持重试安装**：
- ✅ **保留分区和标签**：不删除 `frzr_root` 和 `frzr_efi` 分区
- ✅ **frzr-deploy 自动重新挂载**：通过 `mount -L` 重新挂载分区
- ✅ **显示重试界面**：返回/重试安装/退出 三个选项
- ✅ **详细的日志输出**：每步操作都有清晰的日志记录
...
显示重试界面（返回/重试/退出）
    ↓
用户选择重试 → frzr-deploy 自动重新挂载 → 继续安装 ✅
```

#### Bootstrap 阶段取消（frzr-bootstrap）

**执行的清理操作**：
1. ✅ **终止进程**：终止 frzr-bootstrap 进程
2. ✅ **检查并清理挂载点**：如果存在则卸载
3. ✅ **重置状态标志**：清除 `bootstrap_completed` 标志
4. ✅ **跳转到完成页面**：显示取消状态

**注意事项**：
- ⚠️ 磁盘可能处于部分初始化状态
- 💡 建议重新选择"全新安装"模式来完全重新初始化磁盘

#### 设计原则

1. **渐进式清理**：终止进程 → 等待释放 → 卸载挂载点 → 清理子卷 → 清理临时文件
2. **安全的卸载顺序**：深层优先（efi → boot → root）
3. **多层回退策略**：正常卸载失败 → 强制卸载 → lazy unmount
4. **支持重试**：保留分区，允许 frzr-deploy 重新挂载
...
正在清理挂载点...
  卸载: /tmp/frzr_root/boot/efi
  ✓ 已卸载
  卸载: /tmp/frzr_root/boot
  ✓ 已卸载
  卸载: /tmp/frzr_root
  ✓ 已卸载
✓ 挂载点清理完成
...
正在清理未完成的系统部署...
  删除未完成的部署: deployments/skorionos-50-4_abc123-gnome
  ✓ 已删除
✓ 已清理 1 个未完成的部署
...
### 取消机制测试 🆕
5. [ ] Bootstrap 阶段取消（进程终止、挂载清理）
6. [ ] Install 阶段取消（进程终止、挂载清理、子卷清理）
7. [ ] 取消后重试安装（直接重试，无需重新 Bootstrap）
8. [ ] 取消后返回重新配置（修改选项后重新安装）
9. [ ] 取消时的日志输出（每步操作清晰可见）
10. [ ] 多次取消重试（稳定性测试）
...
### 后安装测试
14. [ ] 网络配置复制（安装后 WiFi 保留）
15. [ ] Steam 首次启动（无需额外配置）
16. [ ] post_install 脚本修改（Steam 会话、steamos-update）

### 清理机制测试
17. [ ] 临时挂载清理（本地安装完成后）
18. [ ] 异常退出清理（Ctrl+C、断电等）

---

## 🔄 更新历史

### 最近完成（2025-11-01 第十二次更新）🆕

1. ✅ **完整取消机制**
   - **Install 阶段（frzr-deploy）**：
     - 优雅终止进程（SIGTERM → SIGKILL）
     - 逆序卸载挂载点（efi → boot → root）
     - 多层回退卸载策略（normal → force → lazy unmount）
     - 清理未完成的 btrfs 子卷（避免磁盘空间浪费）
     - 清理临时文件（*.img.*）
     - 显示重试界面（返回/重试安装/退出）
     - 支持直接重试（frzr-deploy 自动重新挂载）
   - **Bootstrap 阶段（frzr-bootstrap）**：
     - 终止 frzr-bootstrap 进程
     - 条件清理挂载点（仅当存在时）
     - 重置内部状态标志
     - 显示重试界面
   - **用户体验**：
...
### 最近完成（2025-10-31 第十一次更新）

1. ✅ **时区自动检测与配置** 🆕
   - 创建 `timezone_utils.py` 模块，实现多层级时区检测
   - **三层获取机制**（优先级递减）：
     1. **现有系统继承**：Live ISO 启动时从 frzr_root 分区复制时区配置（最优先）
     2. **网络异步检测**：连接网络后后台检测，不阻塞流程
     3. **Bootstrap 同步检测**：前两者都失败时的回退方案
   - **中国国内优先**：使用真正的中国国内服务（太平洋电脑网、ip.useragentinfo.com、cip.cc），避免代理网络误判
   - **智能检测逻辑**：检测到中国 IP 直接返回 `Asia/Shanghai`（中国统一时区）
   - **国际 IP 地理定位**：使用 3 个免费 API（ipapi.co、ip-api.com、worldtimeapi.org）
   - **准确度**：重装系统 100%（继承）、中国用户 95%+、其他单时区国家 85-95%
   - **回退策略**：现有系统 → 中国检测 → 国际 IP 定位 → systemd 推荐 → UTC
   - **异步检测优化**：网络连接成功后后台异步执行，不阻塞安装流程
   - **智能启动**：已联网（以太网）时页面初始化自动启动，WiFi 连接成功时自动启动
   - **避免重复检测**：检查环境变量和检测标记，已完成则跳过
   - **应用到 Live 环境**：使用 `timedatectl set-timezone`，刷新 Python 时区缓存
   - **状态栏实时更新**：添加 `StatusBar.refresh_timezone()` 方法，时区变更后立即更新显示
   - **复制到目标系统**：创建 `/etc/localtime` 符号链接和 `/etc/timezone` 文件
   - **环境变量传递**：通过 `INSTALLER_TIMEZONE` 在安装流程中传递
   - **容错处理**：任何步骤失败都不会阻塞安装，只记录警告
...
### 2025-11-01 (第十二次更新)
- ✅ **取消按钮无实际作用** → 实现完整的进程终止和资源清理机制
- ✅ **取消后进程继续运行** → 使用 SIGTERM → SIGKILL 强制终止进程
- ✅ **取消后挂载点未清理** → 逆序卸载所有挂载点（多层回退策略）
- ✅ **取消后磁盘空间未回收** → 删除未完成的 btrfs 子卷和临时文件
- ✅ **取消后无法重试安装** → 保留分区和标签，利用 frzr-deploy 自动重新挂载
- ✅ **取消操作无用户反馈** → 详细的日志输出，展示每一步清理操作
- ✅ **取消后直接返回上一页** → 改为显示重试界面（返回/重试/退出）
